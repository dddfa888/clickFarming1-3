    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:cooperate:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:cooperate:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
...
<script>
import { listCooperate, getCooperate, delCooperate, addCooperate, updateCooperate } from "@/api/setting/settingDevCooperate"
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCooperate(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addCooperate(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除设置发展合作编号为"' + ids + '"的数据项？').then(function() {
        return delCooperate(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/cooperate/export', {
        ...this.queryParams
      }, `cooperate_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
