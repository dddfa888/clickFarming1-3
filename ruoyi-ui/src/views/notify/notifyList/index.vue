...
<script>
import { listNotify, getNotify, delNotify, addNotify, updateNotify } from "@/api/notify/notify"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除通知编号为"' + uids + '"的数据项？').then(function() {
        return delNotify(uids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/notify/export', {
        ...this.queryParams
